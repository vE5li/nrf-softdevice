...
struct CharacteristicArgs {
    ...
    security: Option<SecurityMode>,
    #[darling(default)]
    initial_value: Option<syn::Ident>,
    #[darling(default)]
...
pub fn gatt_service(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for ch in &chars {
        ...
        let initial_value = match &ch.args.initial_value {
        ...
        code_build_chars.extend(quote_spanned!(ch.span=>
            let mut characteristic_builder = {
                ...
                service_builder.add_characteristic(#uuid, attr, metadata)?
            };

            #descriptor

            let #char_name = characteristic_builder.build();
        ));
